...
def train(model, train_loader, test_loader, experiment):
    ...
    for epoch in range(model.opt.starting_epoch_count+1, model.opt.n_epochs+1): # opt.niter + opt.niter_decay + 1):
        ...
        if (epoch==1):
            min_eval_loss, median_error = valid(model, epoch, test_loader, save_output=False, experiment=experiment)
        else:
            new_eval_loss, new_med_error = valid(model, epoch, test_loader, save_output=False, experiment=experiment)
            if (median_error>=new_med_error):
                stopping_count = stopping_count+1
                median_error = new_med_error
        print("The current stopping count:", stopping_count)
...
def valid(model, epoch, test_loader, experiment, save_output=True, save_name="decoder_test_result", save_dir="", log=True):
...
def test(model, test_loader, experiment, save_output=True, save_name="decoder_test_result", save_dir="", log=True):
    ...
    for i, data in enumerate(test_loader):
        ...
        if opt_exp.n_decoders == 2:
            offset_outputs.extend(off_outputs.data.cpu().numpy())
            labels.extend(data['labels_gaussian_2d'].cpu().numpy())
        error.extend(localization_error(gen_outputs.data.cpu().numpy(),data['labels_gaussian_2d'].cpu().numpy(),scale=0.1))
        total_loss += model.decoder.loss.item()
        if opt_exp.n_decoders == 2:
            total_offset_loss += model.offset_decoder.loss.item()
    ...
    if save_output:
        ...
        save_path = f"{save_dir}/{save_name}.mat"
        hdf5storage.savemat(save_path,
            mdict={"outputs":generated_outputs,"wo_outputs":offset_outputs, "error": error,"labels": labels}, 
            appendmat=True, 
            format='7.3',
            truncate_existing=True)
        print(f"result saved in {save_path}")
