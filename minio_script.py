...
def store_received_data(received_data, bucket_name="wl-data"):
    ...
    try:
        ...
        data_entry = {
            "user_id": [user_id],
            "timestamp": [timestamp],
            "IMU": [json.dumps(data.get("IMU", {}))],
            "GPS": [json.dumps(data.get("GPS", {}))],
            "WiFi": [json.dumps(data.get("WiFi", {}))],
            "Channel": [json.dumps(data.get("Channel", {}))]
        }
...
def main():
    while True:
        print("\nOptions:")
        print("1: Verify if the table (bucket) has been created and list all stored objects.")
        print("2: Store Valid Data Test.")
