...
class IMU_publisher:
    ...
    def subscribe(self, client: mqtt_client):
        def on_message(client, userdata, msg):
            ...
            if len(self.accelerator_list) > 0 and len(self.gyroscope_list) > 0:
                gyro_and_accel = [self.accelerator_list, self.gyroscope_list]
                gyro_timestamp = int(self.gyroscope_list[4].split(".")[1])
                accel_timestamp = int(self.accelerator_list[4].split(".")[1])
                timestamp_diff = abs(accel_timestamp - gyro_timestamp)
                self.accelerator_list = []
                self.gyroscope_list = []
...
def run():
    mqttHandler = IMU_publisher(CLIENT_ID, BROKER, PORT, TOPIC)
