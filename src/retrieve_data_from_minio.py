    ...
    try:
        ...
        if user_id:
            if timestamp is not None:
                ...
                print(
                    f"Retrieving data for user_id: {user_id} and timestamp: {timestamp}"
                )
                ...
                retrieved_data = display_content(
                    "wl-data", user_id
...
def send_data_to_mqtt(
    data, broker=MQTT_BROKER, port=MQTT_PORT, topic=MQTT_OUTPUT_TOPIC
):
    ...
    if data is not None and not data.empty:
        ...
        for entry in data.to_dict(orient="records"):
            ...
            try:
                ...
                if (
                    isinstance(gps_info, dict)
                    and "latitude" in gps_info
                    and "longitude" in gps_info
                ):
                    gps_coordinates_pairs.append(
        ...
        if gps_coordinates_pairs:
            ...
            try:
                ...
                if result.rc == mqtt.MQTT_ERR_SUCCESS:
                    ...
                    print(
                        f"Failed to publish data to MQTT topic '{topic}'. Error code: {result.rc}\n"
                    )
                    print(
                        "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
                    )
